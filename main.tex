...
Additionally we created a Heading component, where the heading level (h1, h2 and h3), the text as well as additional css classes can be set dynamically. Doing that, we are able to style the different headings accordingly and assure consistency throughout the interface.

\subsubsection{Paragraph}
In addition to the heading component, we also created a paragraph component with almost the same purpose. In this case we are able to dynamically generate text with custom css styling.

\subsubsection{HobbySelection} \label{HobbySelectionComponent}d
The HobbySelection component was implemented to display "Additional hobbies" at the hobby page \ref{Hobbies} in the form of a MultiSelect dropdown field. The component dynamically populates hobby options fetched from a JSON file with many different hobbies in it. By managing state changes, the component is able to refresh our Redux store, updating user preferences in real-time. In summary the component helps the user to find his desired hobby if it is not in one of the card blocks "culture", "sports" or "entertainment" \ref{label}.
...
We are proud that we accomplished to develop a individual travel guide for users that try to include their hobbies into their journey easily. Thanks to the enthusiastic participation of our survey respondents and user testers, we were able create a intuitive and effective way to allow users to plan their next travel experience. We were able to deliver users a combination of the must-sees attractions and also a way to invoke their personality and hobbies into their journey. 

Certainly, there are a lots of ways to improve our prototype to become a fully functional web platform for individualized travel guides. A one of this issue we already mentioned in the challenge section of the project details \ref{Challenges}. Other improvements we identified for our travel guide platform, based on the evaluation of the survey, user testing, and our own reflections on the project:
