...
Several lessons were learned during the project. Firstly, we came across on how to set up a Python backend which proved to be quite challenging, especially because we both have never done this before. Additionally, integrating the different APIs was crucial for getting data and the functionality of Guide Guru. Designing prompts for ChatGPT was another significant learning aspect, highlighting the importance of creating effective inputs for natural language processing. Towards the project's conclusion, we reflected that using JSON could have provided improved organization for ChatGPT's responses, thus offering greater flexibility in handling data. Furthermore, we gained experience in handling unpredictable data, specifically the responses from ChatGPT, through the utilization of regular expressions (regex). These lessons will defenitely help us for future projects and we are happy we experienced them.
...
Additionally, we gained valuable knowledge about React as a frontend technology. We learned effective methods for separating content and testing the web application thoroughly. 

These experiences broadened our understanding of both backend and frontend development processes, highlighting the importance of careful planning and iteration to achieve desired outcomes.
